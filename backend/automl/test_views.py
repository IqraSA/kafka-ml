...
class ModelViewTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.number_of_models = 3
        cls.code = ML_CODE

        for model in range(cls.number_of_models):
            MLModel.objects.create(name=f'Model {model}', code=f'Code {model}')
    ...
    def test_delete_fail(self):
        resp = self.client.delete(f'/models/{str(self.number_of_models + 1)}')
    ...
    def test_create(self):
        ...
        resp = self.client.delete(f'/models/{str(obj.id)}')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_update(self):
        ...
        resp = self.client.put(
            f'/models/{str(self.number_of_models)}',
        ...
        resp = self.client.delete(f'/models/{str(self.number_of_models)}')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_create_distributed(self):
        ...
        resp = self.client.delete(f'/models/{str(father.id)}')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

        resp = self.client.delete(f'/models/{str(child.id)}')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
...
class ConfigurationViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.configuration = Configuration.objects.create(
            name='Configuration', description="Description"
        )

        for model in range(cls.number_of_models):
            model = MLModel.objects.create(name=f'Model {model}', code=f'Code {model}')
            cls.models.append(model)
            cls.configuration.ml_models.add(model)
        cls.configuration.save()
    ...
    def test_update(self):
        ...
        resp = self.client.put(
            f'/configurations/{str(self.configuration.pk)}',
            content_type='application/json',
            data=data,
        )
    ...
    def test_delete(self):
        resp = self.client.delete(f'/configurations/{str(self.configuration.pk)}')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertTrue(Configuration.objects.all().count() == 0)

    def test_delete_fail(self):
        resp = self.client.delete('/configurations/8')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
...
class ResultViewTest(TestCase):
    ...
    def test_upload_results(self):
        ...
        resp = self.client.post(
            f"/results/{str(self.resultID)}", data, format='multipart'
        )
